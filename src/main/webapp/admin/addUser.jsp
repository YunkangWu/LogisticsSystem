...
<div class="formbody">
    <div class="formtitle"><span>添加新用户</span></div>
    <form action="${pageContext.request.contextPath}/user/add" method="post" onsubmit="return checkForm();">
        <ul class="forminfo">
            <li><label>用户类型</label>
                <label for="userType1" onclick="to_change()"><input type="radio" id="userType1" name="userType" value="2"
                ${(requestScope.user.userType=='2' || requestScope.user.userType==null)?'checked=\"checked\"':''} />企业用户</label>
                <label for="userType2" onclick="to_change()"><input type="radio" id="userType2" name="userType" value="1"
            ...
            <div id="company">
                ...
                <li><label>企业类型</label>
                    <select name="companyType" class="dfinput">
                        ...
                        <option value="4"
                        ${requestScope.user.companyType == '4'?'selected=\"selected\"':''}>物流中介
                            如果您想发布招聘信息，请选择
                        </option>
                    </select>
                </li>
            ...
            <li><input name="" type="submit" class="btn" value="确认添加"/></li>
        </ul>
    </form>
